...
bool CheckStakeKernelHash(unsigned int nBits, const CBlock& blockFrom, unsigned int nTxPrevOffset, const CTransaction& txPrev, const COutPoint& prevout, unsigned int nTimeTx, uint256& hashProofOfStake, uint256& targetProofOfStake, bool fPrintProofOfStake)
{
    ...
    {
        if(fDebug)
            printf("ERROR: CheckStakeKernelHash():GetKernelStakeModifier failed\n");
        return false;
    }
    ss << nStakeModifier;
    ...
    {
        if(fDebug)
            printf("ERROR: CheckStakeKernelHash():proof-of-stake hash don't meets target protocol\nhashProofOfStake: %s\nbnCoinDayWeight: %s\nbnTargetPerCoinDay: %s\nbnCoinDayWeight * bnTargetPerCoinDay: %s\n",hashProofOfStake.ToString().c_str(),
    ...
    {
        if(fDebug || fPrintProofOfStake)
            printf("CheckStakeKernelHash: Yeal!!!, hashProofOfStake meets target protocol\n");
...
{
    //bug464289
    if(pindex && pindex->nHeight == 464289)
        return 1872435112; //0x6F9B17A8

    //bug464290
    //if(pindex && pindex->nHeight == 464290)
    //    return 2654960433; //0x9e3f7b31
